...
- `--json` — instruction-based JSON-only response (best-effort). Use
  `--json-schema` for a stronger guarantee.
- `--json-schema PATH` — load a JSON schema from PATH and enforce it via
  ...
  it into every nested object (including `type:["object","null"]` array forms).
  Schema is deepcopied before mutation so callers reusing the same dict are
  unaffected. Stdout will be valid JSON parseable by `json.loads()`.
